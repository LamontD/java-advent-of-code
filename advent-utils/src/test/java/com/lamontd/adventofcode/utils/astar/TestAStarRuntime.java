...
public class TestAStarRuntime {
    ...
    private static long runTestForGraphSize(int numNodes, int numEdges) {
        List<NodeWithCoordinate> shortestPath = null;
        long time = 0;
        while (shortestPath == null) {
            NodeWithCoordinate[] nodes = TestGraphFactory.createNodes(numNodes);

            ValueGraph<NodeWithCoordinate, Double> graph =
                    TestGraphFactory.createGraph(nodes, numEdges);

            NodeWithCoordinate source = nodes[0];
            NodeWithCoordinate target = nodes[1];
            ...
            HeuristicForNodesWithCoordinate heuristic =
                    new HeuristicForNodesWithCoordinate(graph, target);
