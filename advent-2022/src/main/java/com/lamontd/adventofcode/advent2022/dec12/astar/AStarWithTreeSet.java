 ...
 *
 * Modified from original code by happycoders.eu to add support for illegal paths in directed graphs
 *
    ...
    public static <N extends Comparable<N>> List<N> findShortestPath(
        ...
        while (!queue.isEmpty()) {
            ...
            for (N neighbor : neighbors) {
                ...
                Optional<Double> cost = graph.edgeValue(node, neighbor);
                if (!cost.isPresent()) {
                    // In this case, it's a directed graph -- so not having a cost is a-okay!
                    continue;
                }
                double totalCostFromStart = nodeWrapper.getTotalCostFromStart() + cost.get();
