...
public class PassportBuilderTester {
    ...
    public void testParsingEntries() {
        ...
        Assert.assertEquals("gry", builder.build().getEyeColor());
        builder.parseEntry("pid:860033327");
        Assert.assertEquals("860033327", builder.build().getPassportId());
        builder.parseEntry("cid:147");
        Assert.assertEquals("147", builder.build().getCountryId());
        builder.parseEntry("eyr:2020");
        Assert.assertEquals(2020, (int) builder.build().getExpirationYear());
        builder.parseEntry("hcl:#fffffd");
        Assert.assertEquals("#fffffd", builder.build().getHairColor());
        builder.parseEntry("iyr:2017");
        Assert.assertEquals(2017, (int) builder.build().getIssueYear());
        builder.parseEntry("hgt:183cm");
        Assert.assertEquals("183cm", builder.build().getHeight());
    }
